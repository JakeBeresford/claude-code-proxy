...
def convert_anthropic_to_litellm(anthropic_request: MessagesRequest) -> Dict[str, Any]:
    ...
    # Extract clean model name for checking
    ...
    # Handle models that only support temperature=1
    temperature_restricted_models = "gpt-5" in clean_model.lower()

    if temperature_restricted_models and temperature != 1.0:
        logger.debug(
            f"Adjusting temperature from {temperature} to 1.0 for {clean_model} model"
        )
        temperature = 1.0
    ...
    if "gpt-5" in clean_model.lower():
        ...
        del litellm_request["max_tokens"]
        logger.debug(f"Using max_completion_tokens for Azure model: {clean_model}")
    else:
